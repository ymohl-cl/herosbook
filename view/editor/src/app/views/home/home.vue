  ...
  <v-container>
		<div v-if="!createNewBookDisplay">
			...
			<div v-if="books.length > 0">
				<h1>Books list</h1>
				<v-card class="mx-auto" max-width="344" outlined v-for="book in books" v-bind:key="book.identifier">
			...
			<div class="container pa-3" :elevation="1" v-if="books.length == 0">
				<div class="flex">
					...
					<span class="subheading mb-2">Vous allez pouvoir donner vie à votre imagination en écrivant des livres dont vous êtes le héros !</span>
					<v-divider></v-divider>
					<div class="title mb-3">Vous êtes prêt ?</div>
					<div class="my-2">
						<v-btn large color="primary" @click="createNewBookDisplay = true">Commencer l'écriture</v-btn>
					</div>
				</div>
			</div>
		</div>
