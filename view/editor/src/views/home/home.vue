  ...
  <div>
    ...
    <v-container v-if="session.books.length > 0">
      ...
      >
        ...
        <v-card-text>
          <p  style="display: inline" class="font-weight-black">Genre: </p><p style="display: inline">{{ book.genre }}</p>
          <br />
          <p  style="display: inline" class="font-weight-black">Creation date: </p><p style="display: inline">Fictive date</p>
        </v-card-text>
        <v-card-actions>
          <v-btn color="orange" @click="goBook(book.identifier)">Edit</v-btn>
          <v-btn color="error" @click="openDeleteDialog(book.identifier)">Delete</v-btn>
        </v-card-actions>
      </v-card>
    </v-container>
    <v-container v-else-if="session.books.length == 0">
      <v-container>
        <p class="text-center">
